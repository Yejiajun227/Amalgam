...
from amalgam.util import math_func
from amalgam.secure_protocol.paillier.fixedpoint import FixedPointNumber
...
class PaillierKeypair(object):
    ...
    def generate_keypair(n_length=1024):
        ...
        while n_len != n_length:
            p = math_func.get_prime(n_length // 2)
            q = p
            while q == p:
                q = math_func.get_prime(n_length // 2)
            n = p * q
            n_len = n.bit_length()
...
class PaillierPublicKey(object):
    ...
    def apply_obfuscator(self, ciphertext, random_value=None):
        ...
        obfuscator = math_func.powmod(r, self.n, self.nsquare)
    ...
    def raw_encrypt(self, plaintext, random_value=None):
        ...
        if plaintext >= (self.n - self.max_int) and plaintext < self.n:
            ...
            ciphertext = math_func.invert(neg_ciphertext, self.nsquare)
        else:
...
class PaillierPrivateKey(object):
    ...
    def __init__(self, public_key, p, q):
        ...
        self.q_inverse = math_func.invert(self.q, self.p)
    ...
    def h_func(self, x, xsquare):
        ...
        """
        return math_func.invert(self.l_func(math_func.powmod(self.public_key.g,
                                                             x - 1, xsquare), x), x)
    ...
    def raw_decrypt(self, ciphertext):
        ...
        mp = self.l_func(math_func.powmod(ciphertext,
                                          self.p - 1, self.psquare),
                         self.p) * self.hp % self.p

        mq = self.l_func(math_func.powmod(ciphertext,
...
class PaillierEncryptedNumber(object):
    ...
    def __mul__(self, scalar):
        ...
        if plaintext >= self.public_key.n - self.public_key.max_int:
            ...
            neg_c = math_func.invert(self.ciphertext(False), self.public_key.nsquare)
            neg_scalar = self.public_key.n - plaintext
            ciphertext = math_func.powmod(neg_c, neg_scalar, self.public_key.nsquare)
        else:
            ciphertext = math_func.powmod(self.ciphertext(False), plaintext, self.public_key.nsquare)
    ...
    def __raw_add(self, e_x, e_y, exponent):
        ...
        """
        ciphertext = math_func.mpz(e_x) * math_func.mpz(e_y) % self.public_key.nsquare
